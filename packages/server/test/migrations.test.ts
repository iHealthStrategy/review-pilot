...
test("migrations: DDL creates every entity table", () => {
  ...
  // 0009 links accounts to the external IdP subject.
  assert.match(ddl, /ALTER TABLE users ADD COLUMN external_id\b/);
  // 0010 adds the IdP display name.
  assert.match(ddl, /ALTER TABLE users ADD COLUMN name\b/);
});
